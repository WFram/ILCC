...
elif params['LiDAR_type'] == 'vlp16_puck_lite':
    h_coef = 2 * np.sin(np.deg2rad(0.3) / 2)
    v_coef = 2 * np.sin(np.deg2rad(2) / 2.)
elif params['LiDAR_type'] == ('ouster' or 'rs_bpearl'):
    h_coef = 2 * np.sin(np.deg2rad(float(params["horizontal_angle"])) / 2)
    v_coef = 2 * np.sin(np.deg2rad(float(params["vertical_angle"])) / 2.)
